...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        device = torch.device(
            "cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu") #device = cuda
    ...
    print("unilm have {} paramerters in total".format(sum(x.numel() for x in model.parameters())))
    ...
    print("optimizer_grouped_parameters",len(optimizer_grouped_parameters))
    if args.fp16:
        try:
    ...
    if args.do_train:
        ...
        if recover_step:
            ...
            start_epoch = 1
        print("000000",args.local_rank,start_epoch, int(args.num_train_epochs)+1)
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                ...
                loss_tuple = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next, masked_pos=masked_pos, masked_weights=masked_weights, task_idx=task_idx, masked_pos_2=oracle_pos, masked_weights_2=oracle_weights, masked_labels_2=oracle_labels, mask_qkv=mask_qkv)

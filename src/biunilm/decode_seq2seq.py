...
import sys
sys.path.append("/home/yuerxin/unilm/src/")
...
def main():
    ...
    #print("222 bi_uni_pipeline", len(bi_uni_pipeline))
    amp_handle = None
    ...
    #print("333 type_vocab_size", type_vocab_size)
    mask_word_id, eos_word_ids, sos_word_id = tokenizer.convert_tokens_to_ids(
        ["[MASK]", "[SEP]", "[S2S_SOS]"])
    #print("444 mask_word_id, eos_word_ids, sos_word_id", mask_word_id, eos_word_ids, sos_word_id)
    ...
    #print("555 forbid_ignore_set, not_predict_set", forbid_ignore_set, not_predict_set)
    print("666 glob.glob(args.model_recover_path.strip())", args.model_recover_path.strip())
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        logger.info("***** Recover model: %s *****", model_recover_path)
        model_recover = torch.load(model_recover_path)
        print("model_recover", type(model_recover),len(model_recover))
        total = 0
        for name, parameters in model_recover.items():
            print(name, ':', parameters.size())
            print(np.prod(parameters.size()))
        ...
        # print("666 input_lines", type(input_lines), input_lines[0], len(input_lines[0][1]))
        ...
        #print("888 output_lines", output_lines, len(output_lines))
            ...
            while next_i < len(input_lines):
                ...
                with torch.no_grad():
                    batch = seq2seq_loader.batch_list_to_batch_tensors(
                        instances)
                    #print("No.10 batch", len(batch), len(batch[0]))
                    
                    batch = [
                        t.to(device) if t is not None else None for t in batch]
                    input_ids, token_type_ids, position_ids, input_mask, mask_qkv, task_idx = batch
                    #print("No.11 token_type_ids", len(token_type_ids), len(token_type_ids[0]), token_type_ids[0])
                    
                    traces = model(input_ids, token_type_ids, position_ids, input_mask, task_idx=task_idx, mask_qkv=mask_qkv)
                    if args.beam_size > 1:
                        traces = {k: v.tolist() for k, v in traces.items()}
                        output_ids = traces['pred_seq']
                        # print("No.12 output_ids", len(output_ids), output_ids[0], len(output_ids[0]))
                        # print("No.13 traces", len(traces), traces.keys(), traces["scores"][0])

                        # print("No.14 traces scores",  len(traces["scores"][0]), traces["scores"][0])
                        # print("No.15 traces wids,", len(traces["wids"][0]), traces["wids"][0])
                        # print("id 1015 = ", tokenizer.convert_ids_to_tokens([2015,2075,4106,3001]))

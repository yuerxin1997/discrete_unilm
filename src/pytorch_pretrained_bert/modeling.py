...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        ...
        if history_states is None: #都是none
            mixed_query_layer = self.query(hidden_states) #mixed_query_layer = [batch,192,768]
            mixed_key_layer = self.key(hidden_states)
            mixed_value_layer = self.value(hidden_states)
        else:
            # print("history_states", history_states.size())
            x_states = torch.cat((history_states, hidden_states), dim=1)
            mixed_query_layer = self.query(hidden_states)
            mixed_key_layer = self.key(x_states)
            mixed_value_layer = self.value(x_states)

        query_layer = self.transpose_for_scores(mixed_query_layer, mask_qkv) #[16,12,192,64] 其中12是attention_head_num, 64是attention_hidden_state
        ...
        # print("mixed_query_layer",mixed_query_layer.size())
        # print("query_layer",query_layer.size())
        # print("key_layer",key_layer.transpose(-1, -2).size())
        attention_scores = torch.matmul(
            query_layer / math.sqrt(self.attention_head_size), key_layer.transpose(-1, -2))
        # print("attention_scores_1",attention_scores.size())
        if self.seg_emb is not None: #seg_emb = none
        ...
        # print("attention_scores_2",attention_scores.size())
        # print("attention_mask", attention_mask.size())
        attention_scores = attention_scores + attention_mask #attention_mask 就是把所有padding的部分变成-10000，有字的地方变成0
        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        # print("self.uni_debug_flag", self.uni_debug_flag)
        if self.uni_debug_flag: #false
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        if pretrained_model_name in PRETRAINED_MODEL_ARCHIVE_MAP:
            archive_file = PRETRAINED_MODEL_ARCHIVE_MAP[pretrained_model_name] # archive_file =  https://s3.amazonaws.com/models.huggingface.co/bert/bert-base-cased.tar.gz
        else:
        ...
        tempdir = None
        if os.path.isdir(resolved_archive_file): #判断是不是一个文件夹，这里不是
            ...
            serialization_dir = tempdir #这个文件夹下有两个文件：bert_config.json, pytorch_model.bin
        # Load config
        if ('config_path' in kwargs) and kwargs['config_path']:
            config_file = kwargs['config_path']
        else:
            config_file = os.path.join(serialization_dir, CONFIG_NAME) #CONFIG_NAME = bert_config.json
        config = BertConfig.from_json_file(config_file)

        # define new type_vocab_size (there might be different numbers of segment ids)
        if 'type_vocab_size' in kwargs:
            config.type_vocab_size = kwargs['type_vocab_size'] #原来是2 现在是6
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        # next_sentence_label = [batch_size]  都是-1
        # task_id = [batch_size] 都是3
        # masked_lm_labels = [batch,64]
        # masked_pos = [batch_size, 64]
        # mask_weights = [batch_size, 64]
        # masked_pos_2 = None
        # masked_weights_2 = None
        # masked_labels_2 = None
        # mask_qkv = none 
        
        if token_type_ids is None and attention_mask is None:#不进入
            task_0 = (task_idx == 0)
            task_1 = (task_idx == 1)
            task_2 = (task_idx == 2)
            task_3 = (task_idx == 3)
        ...
        prediction_scores_masked, seq_relationship_score = self.cls(
            sequence_output_masked, pooled_output, task_idx=task_idx)
        if self.crit_mask_lm_smoothed: #进入
        ...
        if self.cls2 is not None and masked_pos_2 is not None: #不进入
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, mask_qkv=None, task_idx=None):
        ...
        # mask_qkv = none
        # task_id = [batch_size] 都是3 
        #extended_attention_mask = [bath,1,192,192]
